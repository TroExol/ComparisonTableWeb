    ...
    async (dispatch, getState) => {
        ...
        try {
            ...
            const formattedItems = lootfarm.reduce((acc, lootfarmItem) => {
                ...
                const profit = calcProfit(itradeItem.price, lootfarmItem.price / 100 * 0.95);

                acc.push({
                    name: lootfarmItem.name,
                    priceLootfarm: lootfarmItem.price / 100,
                    priceLootfarmWithCommission: Number((lootfarmItem.price / 100 * 0.95).toFixed(2)),
                    priceItrade: itradeItem.price,
                    profit,
                    lootfarmHave: lootfarmItem.have,
                    lootfarmMax: lootfarmItem.max,
            ...
            const filteredItems = formattedItems.reduce((acc, item) => {
                ...
                if (settings.isHideOverstocks && item.lootfarmMax - item.lootfarmHave <= 0) {
                    return acc;
                }
